...
export function countryOptions(locale: Locale, search = ''): [string, string][] {
...
export function hasMarketCoverage(markets: Market[], countryCode: string, city = ''): boolean {
  if (!countryCode) return false;
  const c = countryCode.toUpperCase();
  return markets.some((m) => {
    const mCountry = (m.countryCode ?? 'PK').toUpperCase();
    if (mCountry !== c) return false;
    if (city) {
      const mCity = (m.city ?? m.area ?? 'Lahore').toLowerCase();
...
export function getAvailableCities(markets: Market[], countryCode: string): string[] {
  if (!countryCode) return [];
  const c = countryCode.toUpperCase();
  const matching = markets.filter((m) => (m.countryCode ?? 'PK').toUpperCase() === c);
...
export function getAvailableDays(markets: Market[], countryCode: string, city = ''): string[] {
  if (!countryCode) return [];
  const c = countryCode.toUpperCase();
  const matching = markets.filter((m) => {
    ...
    if (mCountry !== c) return false;
    if (city) {
      const mCity = (m.city ?? 'Lahore').toLowerCase();
      return mCity === city.toLowerCase();
    }
    return true;
  });
  return Array.from(new Set(matching.map((m) => m.day))).filter(Boolean).sort();
}
...
export function formatMarketMoney(
  ...
  try {
    const loc = locale === 'ur' ? 'ur-PK' : currency === 'PKR' ? 'en-PK' : currency === 'GBP' ? 'en-GB' : 'en-US';
