...
describe("API transport (synthetic fixtures, not production endpoints)", () => {
  it("requires an approved base and blocks origin/path escape", async () => {
    ...
    expect(() =>
      createApiClient("https://user:password@example.test"),
    ).toThrow();
  ...
  it("handles empty success and preserves unknown error structure", async () => {
    ...
    await expect(client.request("fixture")).rejects.toMatchObject({
      status: 422,
      body: { custom: "invalid" },
    });
  });
  ...
  it("distinguishes malformed JSON, HTTP failures, and network failures", async () => {
    ...
    await expect(client.request("fixture")).rejects.toBeInstanceOf(
      ApiHttpError,
    );
    await expect(client.request("fixture")).rejects.toThrow(
      "network unavailable",
    );
  });
});
